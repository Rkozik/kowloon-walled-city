...
export class Unit {
  ...
  draw() {
    if (this.canConstructAboveGround()) {
      if (this.neighbors.southernNeighbor().type !== null) {
        if (
          this.neighbors.southernNeighbor().type !== "entrance" &&
  ...
  handleSouthernNeighborBelowGround() {
    if (
      this.neighbors.southernNeighbor().type === null ||
      this.neighbors.southernNeighbor().type === "basement"
    ) {
      this.utils.addBackgroundImage(
        this.neighbors.southernNeighbor(),
        "img/rooms/basement-bg.png"
      );
    }
  }
  ...
  handleEasternNeighborAboveGround() {
    if (this.neighbors.easternNeighbor().type === null) {
      this.neighbors
        .easternNeighbor()
        .domElement.classList.add("balcony-top-east");
      this.neighbors.easternNeighbor().type = "balcony";

      if (
        this.neighbors.southEasternNeighbor().domElement.classList[1] ===
        "balcony-top-east"
      ) {
        this.neighbors
          .southEasternNeighbor()
          .domElement.classList.remove("balcony-top-east");
        this.neighbors
          .southEasternNeighbor()
          .domElement.classList.add("balcony-east");
      }
    } else if (this.neighbors.easternNeighbor().type === "balcony") {
      this.neighbors.easternNeighbor().domElement.className = "node";
      this.neighbors
        .easternNeighbor()
        .domElement.classList.add("clothes-line-n2n");
  ...
  handleEasternNeighborBelowGround() {
    if (
      ...
      this.utils.addBackgroundImage(
        ...
        "img/rooms/basement-wall-east.png"
      );
      this.neighbors.easternNeighbor().type = "basement";
    } else if (
      this.neighbors.easternNeighbor().domElement.classList[1].includes("wall")
    ) {
      this.utils.addBackgroundImage(
        this.neighbors.easternNeighbor(),
        "img/rooms/basement-wall-n2n.png"
      );
      this.neighbors.easternNeighbor().type = "basement";
    }
  }
  ...
  handleWesternNeighborAboveGround() {
    if (this.neighbors.westernNeighbor().type === null) {
      this.neighbors
        .westernNeighbor()
        .domElement.classList.add("balcony-top-west");
  ...
  handleWesternNeighborBelowGround() {
    if (
      ...
      this.utils.addBackgroundImage(
        ...
        "img/rooms/basement-wall-west.png"
      );
      this.utils.addBackgroundImage(
        this.neighbors.southWesternNeighbor(),
        "img/rooms/basement-wall-southwest.png"
      );
      this.neighbors.westernNeighbor().type = "basement";
      this.neighbors.southWesternNeighbor().type = "basement";
    } else if (
      this.neighbors.westernNeighbor().domElement.classList[1].includes("wall")
    ) {
      this.utils.addBackgroundImage(
        this.neighbors.westernNeighbor(),
        "img/rooms/basement-wall-n2n.png"
  ...
  handleSouthEasternNeighborBelowGround() {
    if (
      ...
      this.utils.addBackgroundImage(
        this.neighbors.southEasternNeighbor(),
        "img/rooms/basement-wall-southeast.png"
      );
  ...
  handleSouthWesternNeighborAboveGround() {
    if (
      this.neighbors.southWesternNeighbor().domElement.classList[1] ===
      "balcony-top-west"
    ) {
      this.neighbors
        .southWesternNeighbor()
        .domElement.classList.remove("balcony-top-west");
      this.neighbors
        .southWesternNeighbor()
        .domElement.classList.add("balcony-west");
    }
  }
      ...
      if (!this.tower.getCrime(this.node)) {
        switch (this.node.type) {
          case "residential-occupied":
            if (!this.tower.getTenant(this.node).isUnemployed()) {
              this.bank_account.payRent("residential");
            }
            break;
          case "commercial-occupied":
            if (this.tower.getAvailableJobs(this.node).length === 0) {
              this.bank_account.payRent("commercial");
            }
            break;
          case "industrial-occupied":
            if (this.tower.getAvailableJobs(this.node).length === 0) {
              this.bank_account.payRent("industrial");
            }
            break;
          default:
            break;
  ...
  isConnected() {
    setInterval(() => {
      if (!this.checkLobbyConnection()) {
        this.warnRow(this.node);
      }
    }, 5000);
  }
  ...
  warnRow(node) {
    ...
    node.domElement.innerHTML =
      '<div id="' + node.domElement.id + '" class="no-connection"></div>';
    setTimeout(() => {
      node.domElement.innerHTML = "";
    }, 500);
  ...
  canConstructUnderground() {
    return !!(
      ...
      !this.neighbors
        .northernNeighbor()
        .domElement.classList[1].includes("entrance") &&
      this.node.floor_id < 6
    );
  }
  ...
  isOccupied() {
    if (this.node.domElement.classList.length > 1) {
      if (
        this.node.domElement.classList[1].includes("occupied") ||
        this.node.type === "empty" ||
