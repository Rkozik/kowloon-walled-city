export class Demand {
  residential_limit = 20;
  commercial_limit = 17;
  industrial_limit = 19;
  constructor(public residential, public commercial, public industrial) {}

  draw() {
    let residential_percentage = this.roundUpToNext5(
      (this.residential / this.residential_limit) * 100
    );
    let commercial_percentage = this.roundUpToNext5(
      (this.commercial / this.commercial_limit) * 100
    );
    let industrial_percentage = this.roundUpToNext5(
      (this.industrial / this.industrial_limit) * 100
    );
    ...
    residential_demand.setAttribute(
      "style",
      "width:" + residential_percentage + "px;"
    );
    commercial_demand.setAttribute(
      "style",
      "width:" + commercial_percentage + "px;"
    );
    industrial_demand.setAttribute(
      "style",
      "width:" + industrial_percentage + "px;"
    );
  }
